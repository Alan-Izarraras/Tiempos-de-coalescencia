...
library(ape)
valor_ns <- c(0, 0.0028, 0.28, 2.8)

for (a in valor_ns)  {
  for (n in 1:10)  {
    read.tree(paste('tree_' a, 'Ns_SFS_replica', n, '.txt', sep=''))
  

    library(ape)


    lista_arboles <- read.tree(paste('trees_', a, 'Ns_songleton_replica', n, '_ready.txt', sep='')) 
    ...
    singletones <- read.table(paste('trees_', a '_Ns_singleton_replica', n, '.txt'), sep='')
    ...
    write.csv(matriz_conteo_rangos, paste("matriz_conteo_rangos_", a, "Ns_sim", n, ".csv", sep=""))
    write.csv(Prob_rangos_, paste("matriz_probabilidad_", a, "Ns_sim", n, ".csv", sep=""))
    }
