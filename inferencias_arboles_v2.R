...
#Recuerda que como los archivos intermedios no se van a guardar, no es necesario que los nombres especificamente.
#Lo unico que tienes que nombrar especificamente son los archivos de salida y los input. 

Matriz_Ns <- c(0, 10, 20, 30, 40, 50, 60)
Matriz_bootstrap <- c(seq(1, 10)) #No olvidar cambiar esto a 100 para todo el data set. 

#Empieza el for gigante. 
#No necesito que las tablas y matrices tengan un nombre especifico, solo requiero eso para los inputs y outputs.
#Tons el input ya esta cubierto, falta cubrir el output. 

for (b in 1:length(Matriz_Ns))  {
  for (a in 1:length(Matriz_bootstrap))  {
    lista_arboles <- read.tree(paste(Matriz_Ns[b], 'Ns_bootstrap', Matriz_bootstrap[a], '.txt', sep=""))
    #print(lista_arboles)
    ...
    rangos_tiempo <- c(0.000002, 0.000016, 0.000128, 16) #t4 
  #rangos_tiempo <- c(0.000002, 0.000016, 0.000128, 0.001024, 0.008192, 0.065536,  0.524288, 4.194304, 16) #x8
  #rangos_tiempo <- c(0.000002, 0.000020, 0.000200, 0.002000, 0.020000, 0.200000, 2.000000, 16) #x10
  #rangos_tiempo <- c(0.000002, 0.000004, 0.000008, 0.000016, 0.000032, 0.000064, 0.000128, 0.000256, 0.000512, 0.001024, 0.002048, 0.004096, 0.008192, 0.016392, 0.032784, 0.065568, 0.131136, 0.264472, 0.528944, 1.057888, 2.115776, 4.231552, 8.643104, 16) #x2
  #Falta un rango de tiempo alternativo que no llegue hasta el tiempo maximo.  
    ...
    matriz_conteo_rangos <- matrix(nrow=20, ncol=length(rangos_tiempo)) #Se cambia aqui
    ...
    write.csv(matriz_conteo_rangos, paste("matriz_", Matriz_Ns[b], "Ns_conteo_b", Matriz_bootstrap[a], ".csv", sep=""))
